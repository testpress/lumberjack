...
class TestVideoTranscoder(Mixin, TestCase):
    ...
    @mock.patch("apps.jobs.runnables.MainTranscodingExecutor")
    ...
    def test_runnable_should_run_ffmpeg_manager(self, mock_ffmpeg_manager, mock_controller, mock_ouptput_factory):
        mock_ffmpeg_manager().check_status.return_value = ExecutorStatus.Finished
        mock_controller().check_status.return_value = ExecutorStatus.Finished
    ...
    def test_task_should_be_stopped_in_case_of_exception(self, mock_celery_control, mock_controller):
        self.create_output()
        mock_controller().check_status.return_value = ExecutorStatus.Errored
        self.video_transcoder.do_run()
